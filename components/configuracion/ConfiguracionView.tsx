...
interface ConfiguracionViewProps {
    ...
    onCompanyInfoSave: (info: FormData) => Promise<CompanyInfo | void>;
    users: User[];
    roles: Role[];
    offices: Office[];
...
const CompanyInfoSettings: React.FC<{ 
    info: CompanyInfo; 
    onSave: (info: FormData) => Promise<CompanyInfo | void>; 
    canEdit: boolean 
}> = ({ info, onSave, canEdit }) => {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, field: 'logo' | 'login_image') => {
        ...
        if (file) {
            if (field === 'logo') {
                ...
                setBgFile(file);
                setFormData({ ...formData, loginImageUrl: URL.createObjectURL(file) });
            }
        }
    };
    
    const handleRemoveImage = (field: 'logoUrl' | 'loginImageUrl') => {
        if (field === 'logoUrl') setLogoFile(null);
        if (field === 'loginImageUrl') setBgFile(null);
        setFormData(prev => ({...prev, [field]: ''}));
    };

    const handleFetchBcvRate = async () => {
        setIsFetchingRate(true);
        addToast({ type: 'info', title: 'Consultando Tasa', message: 'Obteniendo valor del BCV...' });
        try {
            const response = await axios.get('https://pydolarvenezuela.vercel.app/api/v1/dollar/unit/bcv');
            ...
            addToast({ type: 'error', title: 'Error', message: 'No se pudo obtener la tasa automáticamente.' });
        } finally {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        submissionData.append('rif', formData.rif || '');
        submissionData.append('address', formData.address || '');
        submissionData.append('phone', formData.phone || '');
        submissionData.append('postal_license', formData.postalLicense || '');
        submissionData.append('cost_per_kg', String(formData.costPerKg || 0));
        submissionData.append('bcv_rate', String(formData.bcvRate || 0));
        submissionData.append('tax_rate', String(info.taxRate || 16));

        if (logoFile) {
            submissionData.append('logo', logoFile);
        }
        if (bgFile) {
            submissionData.append('login_image', bgFile);
        }

        try {
            const updatedInfo = await onSave(submissionData);
            if (updatedInfo) {
                 ...
                 <fieldset disabled={!canEdit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                        <div>
                            ...
                            <Button type="button" variant="secondary" onClick={() => logoFileInputRef.current?.click()} className="mb-2" disabled={!canEdit} >
                                <UploadIcon className="w-4 h-4 mr-2" /> Subir Logo...
                            </Button>
                            <input type="file" ref={logoFileInputRef} onChange={(e) => handleFileChange(e, 'logo')} accept="image/png, image/jpeg" className="hidden" />
                            <div className="relative w-full h-20 rounded-lg bg-gray-50 dark:bg-gray-800 flex items-center justify-center p-2 border-2 border-dashed border-gray-300 dark:border-gray-600">
                                {formData.logoUrl ? (
                                    <>
                                        ...
                                        {canEdit && <button type="button" onClick={() => handleRemoveImage('logoUrl')} className="absolute top-1 right-1 bg-red-600 text-white rounded-full p-1 shadow-md hover:bg-red-700 transition-colors" aria-label="Remove logo" >
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                             ...
                             <Button type="button" variant="secondary" onClick={() => bgFileInputRef.current?.click()} className="mb-2" disabled={!canEdit} >
                               <UploadIcon className="w-4 h-4 mr-2" /> Subir Fondo...
                            </Button>
                            <input type="file" ref={bgFileInputRef} onChange={(e) => handleFileChange(e, 'login_image')} accept="image/png, image/jpeg" className="hidden" />
                            <div className="relative w-full h-20 rounded-lg bg-gray-50 dark:bg-gray-800 flex items-center justify-center p-2 border-2 border-dashed border-gray-300 dark:border-gray-600">
                                {formData.loginImageUrl ? (
                                    <>
                                        ...
                                        {canEdit && <button type="button" onClick={() => handleRemoveImage('loginImageUrl')} className="absolute top-1 right-1 bg-red-600 text-white rounded-full p-1 shadow-md hover:bg-red-700 transition-colors" aria-label="Remove background" >
                                            <XIcon className="w-4 h-4" />
                                        </button>}
                                    </>
                                ) : (
