...
import { Report, Invoice, Client, Expense, Office, CompanyInfo, PaymentMethod, Vehicle, Category, ShippingStatus, PaymentStatus } from '../../types';
...
const ReportDetailView: React.FC<ReportDetailViewProps> = ({ report, invoices, clients, expenses, offices, companyInfo, paymentMethods, vehicles }) => {
        ...
        switch(report.id) {
            ...
            case 'reporte_envios_vehiculo':
                ...
                const groupedByVehicleId = vehicleInvoices.reduce((acc, inv) => {
                    ...
                    if (!acc[key]) acc[key] = { vehicle: vehicles.find(v => v.id === key), invoices: [] };
                    acc[key].invoices.push(inv);
                    return acc;
                }, {} as Record<string, { vehicle: Vehicle | undefined, invoices: Invoice[] }>);
                return Object.values(groupedByVehicleId);
            default:
                return [];
        }
    }, [report.id, dateFilteredInvoices, dateFilteredExpenses, companyInfo, clients, paymentMethods, offices, vehicles]);
        ...
        switch(report.id) {
            ...
            case 'gastos_oficina':
             ...
             case 'reporte_kilogramos':
                ...
                break;
             case 'reporte_envios_vehiculo':
                 headers = ["Vehículo", "Placa", "Factura #", "Cliente", "Monto", "Kg"];
                 dataToExport = (sourceData as any[]).flatMap(d => d.invoices.map((inv: Invoice) => ({
                    [headers[0]]: d.vehicle.model, [headers[1]]: d.vehicle.plate, [headers[2]]: inv.invoiceNumber, [headers[3]]: inv.clientName, [headers[4]]: inv.totalAmount, [headers[5]]: calculateInvoiceChargeableWeight(inv)
                 })));
                 break;
            case 'clientes':
                            ...
                            {data.map((d, index) => {
                                ...
                                <Card key={index} className="p-0 overflow-hidden">
                                    <button onClick={() => setExpandedCard(d.vehicle.id)} className="w-full text-left bg-gray-50 dark:bg-gray-800/50 p-4 flex items-center gap-4 hover:bg-gray-100 dark:hover:bg-gray-700/50 transition">
                                        ...
                                        <div className="flex-grow">
                                            <h4 className="font-bold text-lg text-gray-900 dark:text-gray-100">{d.vehicle ? `${d.vehicle.model} - ${d.vehicle.plate}` : 'Vehículo no encontrado'}</h4>
                                            <p className="text-sm text-gray-500 dark:text-gray-400">Conductor: {d.vehicle?.driver || 'N/A'} | Total: {d.invoices.length} facturas | {formatCurrency(totalAmount)} | {totalKg.toFixed(2)} Kg</p>
                                        </div>
