...
const ClientSearchInput: React.FC<ClientSearchInputProps> = ({ clients, onClientSelect, party, guide, onClientChange }) => {
    ...
    useEffect(() => {
        ...
        if (term) {
            // --- CAMBIO AQUÍ: Se añaden verificaciones de seguridad ---
            const filtered = clients.filter(client =>
                (client && client.name && client.name.toLowerCase().includes(term)) ||
                (client && client.idNumber && client.idNumber.toLowerCase().includes(term))
            );
    ...
    return (
        <div className="relative" ref={wrapperRef}>
            <Input
                label={`Buscar Cliente por ${idLabel} o Nombre`}
                value={searchTerm}
                onChange={e => {
