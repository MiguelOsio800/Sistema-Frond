...
const InvoiceDetailView: React.FC<InvoiceDetailViewProps> = ({ isOpen, onClose, invoice, companyInfo, clients, categories }) => {
    ...
    const handleDownloadPdf = () => {
        ...
        pdf.roundedRect(margin, y, boxWidth, 35, 3, 3, 'FD');
        pdf.roundedRect(margin + boxWidth + 10, y, boxWidth, 35, 3, 3, 'FD');
        ...
        pdf.text(`CI/RIF: ${sender.idNumber || ''}`, margin + 5, y + 21);
        pdf.text(`Telf: ${sender.phone || ''}`, margin + 5, y + 26);
        const senderAddrLines = pdf.splitTextToSize(sender.address || '', boxWidth - 10);
        pdf.text(senderAddrLines, margin + 5, y + 31);
        ...
        pdf.text(`CI/RIF: ${receiver.idNumber || ''}`, margin + boxWidth + 15, y + 21);
        pdf.text(`Telf: ${receiver.phone || ''}`, margin + boxWidth + 15, y + 26);
        const receiverAddrLines = pdf.splitTextToSize(receiver.address || '', boxWidth - 10);
        pdf.text(receiverAddrLines, margin + boxWidth + 15, y + 31);
