...
const VehicleShipmentManifest: React.FC<VehicleShipmentManifestProps> = ({
    ...
    const handleDownloadPdf = () => {
        ...
        html2canvas(input, { scale: 2, useCORS: true }).then(canvas => {
            ...
            pdf.save(`remesa_vehiculo_${vehicle.plate}.pdf`);
        });
    };
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Remesa de Carga - Vehículo ${vehicle.plate}`} size="4xl">
            <div id="manifest-to-print" className="bg-white text-gray-900 p-4 md:p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-1 mt-4 text-sm border p-2 rounded-md">
                    <div><strong className="block text-gray-500 text-xs">VEHÍCULO:</strong> {vehicle.model}</div>
                    <div><strong className="block text-gray-500 text-xs">PLACA:</strong> {vehicle.plate}</div>
