...
const AssignInvoiceModal: React.FC<AssignInvoiceModalProps> = ({ isOpen, onClose, onAssign, vehicle, availableInvoices, allInvoices, offices }) => {
    ...
    const { currentLoadKg, remainingCapacity, selectedInvoicesWeight, exceedsCapacity } = useMemo(() => {
        ...
        // CAMBIO: Se usa 'capacity_kg' para que coincida con los datos del backend
        const remainingCapacity = vehicle.capacity_kg - currentLoadKg;
    ...
    return (
        // CAMBIO: Se usa 'license_plate' para mostrar la placa en el título
        <Modal isOpen={isOpen} onClose={onClose} title={`Asignar Envíos a ${vehicle.model} - ${vehicle.license_plate}`}>
                    ...
                    return (
                        ...
                        >
                            <div>
                                ...
                                {/* CAMBIO: Se usa la ruta correcta para el nombre del destinatario */}
                                <p className="text-sm text-gray-600 dark:text-gray-300">{invoice.guide.receiver.name}</p>
                            </div>
