...
const AuditLogView: React.FC<AuditLogViewProps> = ({ auditLog, users }) => {
    ...
    const filteredLogs = useMemo(() => {
        return auditLog.filter(log => {
            ...
            // CAMBIO: Se obtiene el nombre del usuario desde el objeto anidado 'user'
            const userName = log.user ? `${log.user.first_name} ${log.user.last_name}`.trim() : 'Sistema';

            if (start && logDate < start) return false;
            if (end && logDate > end) return false;
            // CAMBIO: Se filtra por 'log.user.id'
            if (selectedUserId && log.user?.id.toString() !== selectedUserId) return false;
            if (searchTerm && !(
                ...
                <div className="mt-4 flex flex-col md:flex-row md:flex-wrap gap-4 items-end">
                    ...
                    <div className="w-full md:w-auto md:flex-grow lg:flex-grow-0 lg:w-1/4">
                        <Select label="Usuario" id="user-filter" value={selectedUserId} onChange={e => setSelectedUserId(e.target.value)}>
                            ...
                            {/* CAMBIO: Se usa el nombre completo del usuario para el filtro */}
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {paginatedData.map(log => (
                            <tr key={log.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                ...
                                {/* CAMBIO: Se muestra el nombre completo del usuario desde el objeto 'user' anidado */}
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{log.user ? `${log.user.first_name} ${log.user.last_name}`.trim() : 'Sistema'}</td>
