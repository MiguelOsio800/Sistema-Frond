                    ...
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {paginatedData.map(client => (
                                <tr key={client.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${['J', 'G'].includes(client.id_type) ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300' : 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300'}`}>
                                            {['J', 'G'].includes(client.id_type) ? <BuildingOfficeIcon className="w-4 h-4 mr-1.5"/> : <UserIcon className="w-4 h-4 mr-1.5"/>}
                                            {['J', 'G'].includes(client.id_type) ? 'Empresa' : 'Persona'}
                                        </span>
                                    </td>
                                    {/* --- CAMBIO APLICADO AQUÍ --- */}
                                    <td className="px-6 py-4 whitespace-nowrap font-mono text-sm text-gray-600 dark:text-gray-400">{`${client.id_type}-${client.id_number}`}</td>
                         ...
                         <tfoot className="bg-gray-50 dark:bg-gray-700/50 font-semibold">
                            <tr>
                                <td className="px-6 py-3 text-left text-sm" colSpan={5}>
